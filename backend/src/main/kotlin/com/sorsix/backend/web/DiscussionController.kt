...
class DiscussionController(
    ...
    fun getCommentsByDiscussion(@PathVariable discussionId: Long): ResponseEntity<List<Comment>> {
        return ResponseEntity.ok(discussionService.getCommentsByDiscussion(discussionId))
    }

    @PostMapping("/{discussionId}/comments")
    fun addComment(@PathVariable discussionId: Long, @RequestBody dto: CommentDto): ResponseEntity<Comment> {
